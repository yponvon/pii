...
class PIIRedactor():
    ...
    def __init__(self):
        ...
        # --- Custom Regex Recognizer: Account Numbers ---
        # An account number is always a contiguous 10-digit number (EBS files
        # start 0-8, MSSL files start 9). Bare-10-digit match per the corpus rule.
        account_recognizer = PatternRecognizer(
            supported_entity="ACCOUNT_NUMBER", patterns=[
                Pattern(name="account_number", regex=r"\b\d{10}\b", score=1)
            ]
        )
        # --- NLP Engine (spaCy) ---
        ...
        # PERSON (full names) is provided for free by the spaCy en_core_web_lg
        # NER engine already configured above -- no explicit recognizer needed.
...
class SingaporeAddressRecognizer(EntityRecognizer):
    ...
    SG Address recognizer using:
      ...
      - SG_UNIT_NUMBER: unit numbers (#2-12, #02-012, etc.)
      ...
      - SG_ADDRESS_BLOCK_NUMBER: block numbers (Blk 123A / Block 123)
    ...
    UNIT_CONTEXT_EXCLUDE_RE = re.compile(
        ...
        r"expiry|expire|expires|expired|valid|validity|renewal|"
        r"visa|mastercard|card|credit|debit"
        r")\b",
        re.IGNORECASE,
    )
    ...
    def analyze(
        ...
        if "SG_UNIT_NUMBER" in entities:
            results += self._find_unit_with_context(text)


        if "SG_ADDRESS_BLOCK_NUMBER" in entities:
            results += self._find(text, self.BLOCK_RE, "SG_ADDRESS_BLOCK_NUMBER", self.high_confidence)
...
class GLiNER2Recognizer(EntityRecognizer):
    ...
    DEFAULT_LABEL_MAPPING = {
        ...
        "address":                "SG_ADDRESS",
        "sg_address_unit_number": "SG_UNIT_NUMBER",
        "sg_address_block_number":"SG_ADDRESS_BLOCK_NUMBER",
    ...
    # Generic concept words that GLiNER2 sometimes tags as PII but are not actual values.
    _GENERIC_WORDS: dict = {
        "SG_ADDRESS": {
            "address", "mailing address", "mailing addresses", "office address",
            "residential address", "home address", "billing address", "delivery address",
            "postal address", "registered address", "residency address",
            "correspondence address", "company address", "service address",
            "street address", "new address", "current address", "updated address",
            "contact address", "promise address", "full address", "house address",
        },
        "SG_UNIT_NUMBER": {
            "unit number",
        },
    ...
    @classmethod
    def _is_generic_concept(cls, span_text: str, presidio_entity: str) -> bool:
        """Return True if the span is a generic concept word rather than an actual PII value.

        GLiNER2 occasionally tags role nouns ("director", "boss") as PERSON, or
        phrases like "mailing address" as SG_ADDRESS. These are not redactable values.
        """
        blocked = cls._GENERIC_WORDS.get(presidio_entity, set())
        return span_text.lower().strip() in blocked

    @classmethod
    def _is_implausible(cls, span_text: str, presidio_entity: str) -> bool:
        ...
        with no digits is never valid. Monetary amounts (e.g. "338.03") are never
        valid account numbers.
        """
        digit_required = {"SG_PHONE_NUMBER", "SG_NRIC_FIN", "SG_POSTAL_CODE"}
        if presidio_entity in digit_required and not re.search(r'\d', span_text):
            return True
        # Real NRIC/FIN always contains letters (e.g. S1234567A); pure-numeric spans are false positives
        if presidio_entity == "SG_NRIC_FIN" and not re.search(r'[A-Za-z]', span_text):
            return True
        return False
    ...
    def analyze(
        ...
        Steps:
          1. Call model.extract_entities() with all GLiNER2 labels, include_spans=True,
             and include_confidence=True so the result contains character-level
             start/end positions and each span's real confidence score.
        ...
        for gliner_label, detected_entities in result.get("entities", {}).items():
            ...
            for e in detected_entities:
                ...
                if self._is_label_literal(span_text, gliner_label):
                    continue
                if self._is_generic_concept(span_text, presidio_entity):
                    continue
                ...
                results.append(
                    RecognizerResult(
                        ...
                        score=e.get("confidence", self.threshold),
                    )
                )
