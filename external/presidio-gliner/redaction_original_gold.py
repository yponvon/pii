...
class PIIRedactor():
    ...
    def __init__(self):
        ...
        sp_utility_account_number = PatternRecognizer(
            supported_entity="SP_ACCOUNT_NUMBER", patterns=[
                Pattern(name="sp_account_number", regex=r"\b\d{10}\b", score=1) # 10-digit number; EBS (0-8 starting number), MSSL (9 starting number)
            ]
        )
        ...
        analyzer.registry.add_recognizer(sp_utility_account_number)
    ...
    def redact(self, text: str, entities: Optional[list] = PII_ENTITIES, score_threshold: Optional[float] = 0.5):
        """
        ...
        anonymize_results = self.anonymizer.anonymize(text=text,analyzer_results=pii_detected, operators={"DEFAULT": OperatorConfig("replace", {"new_value": ""})})
